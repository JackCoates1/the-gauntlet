...
export const openapi = {
  ...
  paths: {
    ...
    '/api/scorecards/{runId}/progression': { get: { tags: ['Research'], summary: 'Prior sealed runs for the same coarse agent fingerprint', description: 'Chronological score progression matched with the same browser-family/major-version/platform fingerprint used by the public digest. No new tracking identifier is stored.',
      parameters: [runId],
      responses: { '200': { description: 'Run ordinal, last score, comparable-score delta and historical score percentages.', ...json({ $ref: '#/components/schemas/Progression' }) }, '400': error('Invalid run ID.'), '404': error('Run not found.') } } },
    ...
    '/feed.xml': { get: { tags: ['Research'], summary: 'Atom feed of verified sealed runs', description: 'Live Atom 1.0 feed emitting one entry per sealed run, reusing the /api/recent D1 read and server-side verified flag. Subscribe to watch the leaderboard in real time; cached for one minute.',
      responses: { '200': { description: 'Atom 1.0 feed.', content: { 'application/atom+xml': { schema: { type: 'string' } } } } } } },
    '/api/webhooks': {
      post: { tags: ['Integrations'], summary: 'Register a signed seal webhook', description: 'Store an https:// callback URL and receive a per-subscriber HMAC-SHA256 signing secret (returned exactly once). Every subsequent successful seal fires a POST containing the run ID, score, verified flag and event-chain root, signed as X-Gauntlet-Signature: t=<unix-ts>,v1=<hex hmac of the unix timestamp, a dot, and the JSON body>. Delivery is fire-and-forget with one retry after a short backoff.',
        requestBody: { required: true, content: { 'application/json': { schema: { type: 'object', required: ['url'], properties: { url: { type: 'string', format: 'uri', description: 'https:// callback URL (no credentials, no localhost/private-IP hosts), 12-500 chars.' } } } } } },
        responses: { '200': { description: 'Registered; signing secret returned once.', ...json({ $ref: '#/components/schemas/WebhookRegistration' }) }, '400': error('Invalid or non-https URL.'), '413': error('Payload too large.'), '503': error('Webhook registry unavailable.'), '507': error('Webhook registry is full.') } },
      get: { tags: ['Integrations'], summary: 'List your registered webhooks by signing secret', description: 'Self-service lookup: pass the hex signing secret you received at registration to list the webhook URLs registered under it. Secrets are never returned.',
        parameters: [{ name: 'secret', in: 'query', required: true, schema: { type: 'string', pattern: '^[0-9a-f]{64}$' } }],
        responses: { '200': { description: 'Registered hooks for this secret.', ...json({ $ref: '#/components/schemas/WebhookList' }) }, '400': error('Missing or malformed secret.'), '503': error('Webhook registry unavailable.') } },
    },
    ...
    '/api/community': { get: { tags: ['Research'], summary: 'Aggregate verified community score distribution', description: 'Buckets authentic signed runs by raw score 0 through 13. Median and quartiles use linear interpolation over sorted scores.', responses: { '200': { description: 'Verified score histogram and summary statistics.', ...json({ $ref: '#/components/schemas/CommunityDistribution' }) } } } },
  ...
  components: { schemas: {
    ...
    CommunityDistribution: { type: 'object', required: ['total', 'buckets', 'verifiedRuns', 'median', 'p25', 'p75', 'perfectCount', 'perfectPct', 'generatedAt'], properties: { total: { const: 13 }, buckets: { type: 'array', minItems: 14, maxItems: 14, items: { type: 'object', required: ['score', 'count'], properties: { score: { type: 'integer', minimum: 0, maximum: 13 }, count: { type: 'integer', minimum: 0 } } } }, verifiedRuns: { type: 'integer', minimum: 0 }, median: { type: ['number', 'null'] }, p25: { type: ['number', 'null'] }, p75: { type: ['number', 'null'] }, perfectCount: { type: 'integer', minimum: 0 }, perfectPct: { type: 'integer', minimum: 0, maximum: 100 }, generatedAt: { type: 'string', format: 'date-time' } } },
    ...
    Progression: { type: 'object', required: ['id', 'fingerprint', 'runNumber', 'priorRunCount', 'previous', 'delta', 'previousScores'], properties: { id: uuid, fingerprint: { type: 'string', description: 'Coarse digest fingerprint: browser family, major version and platform.' }, runNumber: { type: 'integer', minimum: 1 }, priorRunCount: { type: 'integer', minimum: 0 }, previous: { type: ['object', 'null'], properties: { score: { type: 'integer', minimum: 0 }, total: { type: 'integer', minimum: 1 }, createdAt: { type: 'string', format: 'date-time' } } }, delta: { type: ['integer', 'null'], description: 'Current raw-score change from the immediately previous run; null when trap totals differ.' }, previousScores: { type: 'array', items: { type: 'number', minimum: 0, maximum: 100 }, description: 'Previous score percentages in chronological order, for a compact sparkline.' } } },
    WebhookRegistration: { type: 'object', required: ['ok', 'url', 'secret'], properties: { ok: { const: true }, url: { type: 'string', format: 'uri' }, secret: { type: 'string', pattern: '^[0-9a-f]{64}$', description: 'HMAC-SHA256 signing secret; returned exactly once at registration.' }, message: { type: 'string' } } },
    WebhookList: { type: 'object', required: ['hooks'], properties: { hooks: { type: 'array', items: { type: 'object', required: ['url', 'created_at'], properties: { url: { type: 'string', format: 'uri' }, created_at: { type: 'string', format: 'date-time' } } } } } },
  } },
};
