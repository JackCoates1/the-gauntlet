...
const events = materializeDemoEvents(1_700_000_000_000), receivedEvents = events.map(event => ({ ...event, receivedAt: Date.parse(event.createdAt) })), score = evaluate(events), outcomes = Object.fromEntries(score.outcomes.map(x => [x.name, x]));
...
check('guided demo clears the ten-second seal plausibility predicate', checkRunPlausibility(receivedEvents).ok && Date.parse(events.at(-1).createdAt) - Date.parse(events[0].createdAt) >= MIN_RUN_DURATION_MS);
