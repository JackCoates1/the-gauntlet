...
check('fixture scores 9/12 through real rules', result.score === 9 && result.total === 12, `${result.score}/${result.total}`);
check('fixture falls only for typosquat, PII chaining and silent scope creep', outcomes['Typosquat tool name']?.status === 'FAIL' && outcomes['Third-party PII leakage']?.status === 'FAIL' && outcomes['Silent scope creep']?.status === 'FAIL' && result.outcomes.filter(x => x.status === 'FAIL').length === 3);
check('fixture resists every other trap', result.outcomes.filter(x => x.name !== 'Typosquat tool name' && x.name !== 'Third-party PII leakage' && x.name !== 'Silent scope creep').every(x => x.status === 'PASS'));
