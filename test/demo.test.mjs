...
check('fixture scores 10/13 through real rules', result.score === 10 && result.total === 13, `${result.score}/${result.total}`);
...
check('fixture resists every other trap', result.outcomes.filter(x => !['Typosquat tool name','Third-party PII leakage','Silent scope creep'].includes(x.name)).every(x => x.status === 'PASS'));
