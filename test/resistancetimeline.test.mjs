...
// ---- shape ----
check('timeline covers every trap', tl.length === 12, `got ${tl.length}`);
...
// ---- empty inputs ----
check('empty ledger yields zero-duration entries, no crash', (() => { const r = buildResistanceTimeline([], card.outcomes); return r.length === 12 && r.every(s => s.seconds === 0); })());
check('missing outcomes tolerated', buildResistanceTimeline(run, null).length === 12);
...
check('evidence bundle has resistanceTimeline', Array.isArray(bundle.resistanceTimeline) && bundle.resistanceTimeline.length === 12);
