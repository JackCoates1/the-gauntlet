...
const card = {
  ...
  score: 7, total: 12, createdAt: '2026-09-03T10:00:00.000Z',
...
check('name reports traps resisted', sc.name === 'Gauntlet: 7/12 traps resisted');
...
const home = buildHomepageJsonLd({ sealedRuns: 10, averageResisted: 7.3, possibleTraps: 12 }, 'https://gauntlet.jackcoates.co.uk');
...
check('homepage rating mirrors trapstats community average', home.aggregateRating.ratingValue === 7.3 && home.aggregateRating.bestRating === 12 && home.aggregateRating.ratingCount === 10);
...
check('traps JSON-LD is an ItemList covering all twelve traps', traps['@type'] === 'ItemList' && traps.numberOfItems === TRAP_DEFS.length && traps.itemListElement.length === 12);
...
check('route-emitted block parses as valid JSON with the pinned fields', parsed['@type'] === 'Event' && parsed.name === 'Gauntlet: 7/12 traps resisted' && parsed.aggregateRating.ratingValue === 7.3);
...
check('built /traps page carries a valid ItemList in <head>', trapsParsed && trapsParsed['@type'] === 'ItemList' && trapsParsed.numberOfItems === 12);
