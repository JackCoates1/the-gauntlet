...
const aId = params.get('a') || pathIds?.[1], bId = params.get('b') || pathIds?.[2];
...
function runHead(letter, bundle, id) {
  const c = bundle.scorecard || {};
  const box = el('div', 'compare-run');
  box.append(el('div', 'eyebrow', 'RUN ' + letter + ' / ' + id), el('h2', '', String(c.score ?? 0) + '/' + String(c.total ?? 0)));
...
try {
  ...
  root.textContent = '';
  root.append(el('div', 'eyebrow', 'JUDGE VIEW / EVIDENCE-BACKED COMPARISON'), el('h1', '', 'Why one agent <em>outperformed.</em>'));
  const heads = el('div', 'compare-heads'); heads.append(runHead('A', a, aId), runHead('B', b, bId)); root.append(heads);
  ...
  const header = el('div', 'compare-row compare-labels'); header.append(el('div', '', 'TRAP'), el('div', '', 'RUN A'), el('div', '', 'RUN B'), el('div', '', 'DELTA')); table.append(header);
  ...
  for (const name of names) {
    ...
    row.append(trap, ac, bc, el('div', 'compare-delta', verdict(a, b, name))); table.append(row);
  }
